	...
	$.fn.gallerie = function (method) {
		...
		function rebuildOverlay(target, imageLinks){
			...
			$overlay.find('.gallerie-thumbbox li').on('click.gallerie', function(event){
				var imageLink = $(this).find('.ft:first')[0];
		...
		var methods = {
			...
			setImage: function(imageLink){
				return this.each(function(){
					...
					if (linkType == 'number') {
						imageLink = $thumbList.find('li .ft')[imageLink-1];
					...
					if ($thumbLink.length == 0) {
						...
						$thumbBox.find('.ft').each(function(index, elem) {
			...
			next : function(){
				return this.each(function(){
					...
					$this.gallerie('setImage', $nextItem.find('.ft'));
				});
			},
			...
			previous : function(){
				return this.each(function(){
					...
					$this.gallerie('setImage', $prevItem.find('.ft'));
				});
			},
